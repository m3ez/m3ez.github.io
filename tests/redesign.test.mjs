...
test('recognition rendering supersedes legacy one-off recognition mutations', () => {
  ...
  assert.doesNotMatch(source, /function normalizeSpecialMentionRecognition\(\)/);
  assert.doesNotMatch(source, /function moveRecognitionBadges\(\)/);
  assert.doesNotMatch(source, /MOVED_RECOGNITION/);
});
